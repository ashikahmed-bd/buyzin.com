...
<template>
  <article
    class="group bg-white rounded overflow-hidden transition border border-border"
  >
      ...
      <div class="space-y-1">
        <div class="flex justify-between text-xs text-gray-500">
          <span
            >{{
              Math.min(
                Math.round((product.sold_count / product.quantity) * 100),
                100,
              )
            }}% Sold</span
          >
        ...
        <div class="w-full h-1.5 bg-gray-200 rounded-full overflow-hidden">
          <div
            class="h-full bg-gradient-to-r from-pink-500 to-red-500 transition-all"
            :style="{
              width:
                Math.min(
                  Math.round((product.sold_count / product.quantity) * 100),
