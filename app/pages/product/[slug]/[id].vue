...
const loadProduct = async () => {
  await productStore.getProduct(route.params.slug, route.params.id);
};
...
const rating = computed(() => Math.floor(product.value?.rating_avg || 0));
    ...
    <div class="max-w-7xl mx-auto px-4 py-6 space-y-6">
      <template v-if="product">
        ...
        <UBreadcrumb
          :items="[
            { label: 'Home', to: '/' },
            { label: product?.category?.name || 'Category', to: '/' },
        ...
        <section class="grid grid-cols-1 lg:grid-cols-[1.05fr_.95fr] gap-4">
          ...
          <div class="w-full">
            <div class="sticky top-24 bg-white p-4 rounded-2xl space-y-6">
              <div class="space-y-2">
                <div class="flex flex-wrap items-center gap-2">
                  ...
                  <span
                    v-if="product?.shipping?.is_shippable"
                    class="inline-flex items-center rounded-full bg-emerald-50 text-emerald-700 px-3 py-1 text-xs font-semibold"
                  >
                ...
                <h1
                  class="text-2xl md:text-4xl font-extrabold tracking-tight text-dark leading-tight"
                >
                  {{ product?.name }}
                </h1>
                    ...
                    <div class="flex text-amber-400">
                      <template v-for="i in 5" :key="i">
                        <IconsIconStarFill
                          ...
                          class="size-4.5"
                        />
                        <IconsIconStar v-else class="size-4.5 text-gray-300" />
                      </template>
                    </div>
              ...
              <div class="flex flex-wrap items-end gap-3">
                <span
                  class="text-3xl md:text-4xl font-black text-primary leading-none"
                >
                ...
                <del
                  ...
                  class="text-base md:text-lg text-gray-400 font-medium"
                >
                ...
                <div class="space-y-3">
                  ...
                  <div class="flex flex-wrap gap-2.5">
                    ...
                    >
                      ...
                      <div class="flex items-center gap-2">
                        <span
                          v-if="variant.base_price > variant.price"
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div
                  v-if="product?.shipping?.is_shippable"
                  class="flex items-start gap-3 rounded-2xl border border-border bg-surface p-3"
                >
                  <UIcon name="i-lucide-truck" class="size-6" />

                  <div>
                    <h4 class="text-sm font-bold text-dark">Fast Delivery</h4>
                    <p class="leading-6 text-body">
      ...
      <template v-else>
        <div
          class="rounded-3xl border border-gray-200 bg-white p-10 text-center text-gray-500 shadow-sm"
        >
          Product not found.
        </div>
      </template>
        ...
        >
          <template #description>
            <div
              ...
              class="text-body max-w-5xl leading-relaxed"
            ></div>
          </template>
